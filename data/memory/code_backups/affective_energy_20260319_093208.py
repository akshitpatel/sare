...
class AffectiveEnergy:
    ...
    def _beauty_energy(self, expr: str, tokens: List[str]) -> float:
        ...
        mirror = stripped == stripped[::-1] if stripped else False
        symmetry = 1.0 if mirror else 0.0
        ...
        if tokens:
            ...
            repeated_occurrences = sum(count - 1 for count in freq.values() if count > 1)
            repeated_token_ratio = repeated_occurrences / float(total_tokens)
            adjacent_ratio = adjacent_repeat_count / float(max(total_tokens - 1, 1))
            repetition = min(0.7 * repeated_token_ratio + 0.3 * adjacent_ratio, 1.0)

        compactness = 1.0 - min(len(expr) / 120.0, 1.0)
    ...
    def _expr_key(expr: str) -> str:
        ...
        expr = re.sub(r"[a-z_][a-z0-9_]*", "v", expr)
        return expr[:120]