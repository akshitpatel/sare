...
class DreamConsolidator:
    ...
    def dream_cycle(self, max_events: int = 20) -> DreamRecord:
        ...
        rec = DreamRecord(self._tick_count, len(reversed_events),
                          len(discoveries), top)
        self._dreams.append(rec)
    ...
    def _collect_surprise_events(self, n: int) -> List[Dict]:
        ...
        if self._predictive_loop:
            ...
            events = [e.__dict__ if hasattr(e, '__dict__') else dict(e)
                      for e in raw[-n:]]
        ...
        if self._world_model:
            ...
            for obs in wm_events[-n:]:
                events.append({"transform": str(obs)[:40],
                                "actual_delta": 0.5, "source": "world_model"})
        return events
        ...
        for i, surprise in enumerate(events):
            ...
            for j, ante in enumerate(window):
                ...
                if conf >= self._MIN_CONF:
                    discoveries.append(CausalDiscovery(
    ...
    def summary(self) -> dict:
        recent_discoveries = [d.to_dict() for d in self._discoveries[-10:]]
        recent_dreams = [r.to_dict() for r in self._dreams[-5:]]
        return {
            "tick_count":        self._tick_count,
            "total_replayed":    self._total_replayed,
            "total_discovered":  self._total_discovered,
            "unique_edges":      len(self._known_edges),
            "recent_discoveries": recent_discoveries,
            "recent_dreams":     recent_dreams,
            "wired": {
                "predictive_loop": self._predictive_loop is not None,
                "causal_graph":    self._causal_graph is not None,
