...
class TaskSchedulerDomain:
    ...
    def generate_multi_step_problems(self, num_problems: int = 3) -> List[GeneratedProblem]:
        ...
        Each generated graph is validated to ensure all step nodes remain reachable
        from a start step via 'precedes' edges before being added to the result list.
        Graphs that fail this check are discarded to prevent unsolvable problems from
        entering the downstream synthesis pipeline.
        """
        if GeneratedProblem is None:
            log.warning("TaskSchedulerDomain: GeneratedProblem not available, returning empty list.")
            return []
        ...
        for _ in range(num_problems):
            try:
                ...
                # Validate that all step nodes remain reachable via precedes edges.
                # Graphs where step removal has disconnected the ordering chain are
                # unsolvable and would cause downstream synth failures; skip them.
                if not _all_steps_reachable(graph):
                    log.debug(
                        "TaskSchedulerDomain.generate_multi_step_problems: "
                        "discarding graph with unreachable steps (actions=%s, missing_edge_idx=%d)",
                        actions,
                        missing_edge_idx,
                    )
                    continue
