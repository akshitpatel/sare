"""Auto-generated from Oracle-validated rule: discovered_pattern_concept_difference_of_squares — Simplification via concept_difference_of_squares (removed 2 nodes)"""
from sare.engine import Transform, Graph
from typing import List, Tuple

class Discovered_DiscoveredPatternConceptDifferenceOfSquares(Transform):
    """Discovered rule: Simplification via concept_difference_of_squares (removed 2 nodes)"""

    def name(self) -> str:
        return 'discovered_pattern_concept_difference_of_squares'

    def match(self, graph: Graph) -> List[dict]:
        matches = []
        for outer in graph.nodes:
            if outer.type == "operator" and outer.label in ('sin', 'sin'):
