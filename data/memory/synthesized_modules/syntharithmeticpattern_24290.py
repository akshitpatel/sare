...
# Generated: 2026-04-16 12:55:17
# Name: syntharithmeticpattern_24290
...
class SynthArithmeticPattern_24290(Transform):
    def name(self) -> str:
        return "syntharithmeticpattern_24290"
    ...
    def apply(self, graph: Graph, context: Dict) -> Tuple[Graph, float]:
        ...
        if "div_node" in context:
            ...
            value = context["value"]
            const_node_id = g.add_node("constant", str(int(value) if value.is_integer() else value), {})
            for e in list(g.edges):
                if e.target == div_node:
                    g.remove_edge(e.id)
                    g.add_edge(e.source, const_node_id, e.relationship_type)
            ...
            g.add_edge(pow_node_id, var_node_id, "base")
            g.add_edge(pow_node_id, exp_node_id, "exponent")
            for e in list(g.edges):
