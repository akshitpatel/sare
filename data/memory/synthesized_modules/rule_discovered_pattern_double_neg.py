"""Auto-generated from Oracle-validated rule: discovered_pattern_double_neg — Simplification via double_neg (removed 2 nodes)"""
from sare.engine import Transform, Graph
from typing import List, Tuple

class Discovered_DiscoveredPatternDoubleNeg(Transform):
    """Discovered rule: Simplification via double_neg (removed 2 nodes)"""

    def name(self) -> str:
        return 'discovered_pattern_double_neg'

    def match(self, graph: Graph) -> List[dict]:
        matches = []
        for outer in graph.nodes:
            if outer.type == "operator" and outer.label in ('neg', 'neg'):
