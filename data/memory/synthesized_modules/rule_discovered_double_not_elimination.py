"""Auto-generated from Oracle-validated rule: discovered_double_not_elimination — not(not(x)) → x"""
from sare.engine import Transform, Graph
from typing import List, Tuple

class Discovered_DiscoveredDoubleNotElimination(Transform):
    """Discovered rule: not(not(x)) → x"""

    def name(self) -> str:
        return 'discovered_double_not_elimination'

    def match(self, graph: Graph) -> List[dict]:
        matches = []
        for outer in graph.nodes:
            if outer.type == "operator" and outer.label in ('not', 'neg'):
