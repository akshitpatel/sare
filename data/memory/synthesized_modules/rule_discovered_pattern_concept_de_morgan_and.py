"""Auto-generated from Oracle-validated rule: discovered_pattern_concept_de_morgan_and — Simplification via concept_de_morgan_and (removed 3 nodes)"""
from sare.engine import Transform, Graph
from typing import List, Tuple

class Discovered_DiscoveredPatternConceptDeMorganAnd(Transform):
    """Discovered rule: Simplification via concept_de_morgan_and (removed 3 nodes)"""

    def name(self) -> str:
        return 'discovered_pattern_concept_de_morgan_and'

    def match(self, graph: Graph) -> List[dict]:
        matches = []
        for outer in graph.nodes:
            if outer.type == "operator" and outer.label in ('and', 'and'):
