"""Auto-generated from Oracle-validated rule: discovered_double_cos_elimination — cos(cos(x)) → x"""
from sare.engine import Transform, Graph
from typing import List, Tuple

class Discovered_DiscoveredDoubleCosElimination(Transform):
    """Discovered rule: cos(cos(x)) → x"""

    def name(self) -> str:
        return 'discovered_double_cos_elimination'

    def match(self, graph: Graph) -> List[dict]:
        matches = []
        for outer in graph.nodes:
            if outer.type == "operator" and outer.label in ('cos', 'cos'):
