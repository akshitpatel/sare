...
# Generated: 2026-04-15 17:47:33
# Name: syntharithmeticpattern_55427
from sare.engine import Transform, Graph

class SynthArithmeticPattern_55427(Transform):
    def name(self):
        return "syntharithmeticpattern_55427"
    ...
    def apply(self, graph, context):
        ...
        if "div" in context:
            ...
            new_const = g.add_node("constant", str(int(result) if result.is_integer() else result), {})
            ...
            g.add_edge(new_pow, var_node.id, "operand")
            exp_const = g.add_node("constant", "2", {})
            g.add_edge(new_pow, exp_const, "operand")
            for e in list(g.edges):
                if e.target == mul_id:
                    g.remove_edge(e.id)
