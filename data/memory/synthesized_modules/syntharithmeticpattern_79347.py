...
# Generated: 2026-04-17 04:13:18
# Name: syntharithmeticpattern_79347
...
class SynthArithmeticPattern_79347(Transform):
    def name(self):
        return "syntharithmeticpattern_79347"
                    ...
                    if num_node and den_node and num_node.type == "constant" and den_node.type == "constant":
                        try:
                            ...
                            if den_val != 0 and num_val % den_val == 0:
                                matches.append({"div_node": n.id, "num": num_node.id, "den": den_node.id})
                        except ValueError:
                            pass
                ...
                if len(children) == 2:
                    ...
                    if left_node and right_node and left_node.type == "variable" and right_node.type == "variable":
                        if left_node.label == right_node.label:
                            matches.append({"mul_node": n.id, "var_left": left_node.id, "var_right": right_node.id})
        return matches
    ...
    def apply(self, graph, context):
        ...
        if "div_node" in context:
            ...
            g.add_edge(new_pow_id, var_left_id, "arg1")
            g.add_edge(new_pow_id, exp_id, "arg2")
            for e in list(g.edges):
                if e.target == mul_id:
                    g.remove_edge(e.id)
