...
# Generated: 2026-04-16 19:30:01
# Name: syntharithmeticpattern_47954
...
class SynthArithmeticPattern_47954(Transform):
    def name(self):
        return "syntharithmeticpattern_47954"
    ...
    def apply(self, graph: Graph, context: Dict) -> Tuple[Graph, float]:
        ...
        new_num_id = g.add_node("constant", str(new_num_val) if new_num_val.is_integer() else str(new_num_val), {})
        new_den_id = g.add_node("constant", str(new_den_val) if new_den_val.is_integer() else str(new_den_val), {})

        for e in list(g.edges):
            if e.source == div_id:
                if e.target == num_id:
                    g.remove_edge(e.id)
                    g.add_edge(div_id, new_num_id, e.relationship_type)
                elif e.target == den_id:
                    g.remove_edge(e.id)
                    g.add_edge(div_id, new_den_id, e.relationship_type)
