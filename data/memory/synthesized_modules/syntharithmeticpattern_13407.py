...
# Generated: 2026-04-16 09:54:14
# Name: syntharithmeticpattern_13407
...
class SynthArithmeticPattern_13407(Transform):
    def name(self) -> str:
        return "syntharithmeticpattern_13407"
    ...
    def apply(self, graph: Graph, context: Dict) -> Tuple[Graph, float]:
        ...
        new_num_id = g.add_node("constant", str(int(new_num) if new_num.is_integer() else new_num), {})
        new_den_id = g.add_node("constant", str(int(new_den) if new_den.is_integer() else new_den), {})

        incoming_edges = g.incoming(div_id)
        for edge in incoming_edges:
            g.add_edge(edge.source, new_num_id if new_den == 1 else div_id, edge.relationship_type)
        ...
        if new_den == 1:
            ...
            for edge in list(g.outgoing(div_id)):
                g.remove_edge(edge.id)
            g.add_edge(div_id, new_num_id, "numerator")
            g.add_edge(div_id, new_den_id, "denominator")
            g.remove_node(num_id)
            g.remove_node(den_id)
            return g, -1.0