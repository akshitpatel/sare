...
# Generated: 2026-04-16 13:16:15
# Name: syntharithmeticpattern_25531
...
class SynthArithmeticPattern_25531(Transform):
    def name(self) -> str:
        return "syntharithmeticpattern_25531"
    ...
    def apply(self, graph: Graph, context: Dict) -> Tuple[Graph, float]:
        ...
        new_op_id = g.add_node("operator", "/", {})
        g.add_edge(new_op_id, new_num_id, "operand")
        g.add_edge(new_op_id, new_den_id, "operand")

        for edge in list(g.edges):
            if edge.target == op_id:
                g.remove_edge(edge.id)
                g.add_edge(edge.source, new_op_id, edge.relationship_type)
