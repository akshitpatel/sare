"""Auto-generated from Oracle-validated rule: discovered_derivative_1_identity — derivative(x, 1) → x"""
from sare.engine import Transform, Graph
from typing import List, Tuple

class Discovered_DiscoveredDerivative1Identity(Transform):
    """Discovered rule: derivative(x, 1) → x"""

    def name(self) -> str:
        return 'discovered_derivative_1_identity'

    def match(self, graph: Graph) -> List[dict]:
        matches = []
        for outer in graph.nodes:
            if outer.type == "operator" and outer.label in ('derivative', 'derivative'):
