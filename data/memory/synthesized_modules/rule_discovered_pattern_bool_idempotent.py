"""Auto-generated from Oracle-validated rule: discovered_pattern_bool_idempotent — Simplification via bool_idempotent (removed 2 nodes)"""
from sare.engine import Transform, Graph
from typing import List, Tuple

class Discovered_DiscoveredPatternBoolIdempotent(Transform):
    """Discovered rule: Simplification via bool_idempotent (removed 2 nodes)"""

    def name(self) -> str:
        return 'discovered_pattern_bool_idempotent'

    def match(self, graph: Graph) -> List[dict]:
        matches = []
        for outer in graph.nodes:
            if outer.type == "operator" and outer.label in ('or', 'or'):
