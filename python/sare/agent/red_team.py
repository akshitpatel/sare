...
class RedTeamAdversary:
    ...
    def __init__(self) -> None:
        ...
        self._round_count         = 0
        self._original_energy_cache: Dict[str, float] = {}  # Cache for original belief energies
    ...
    def run_attack_round(self, top_k: int = 3) -> dict:
        ...
        self._round_count += 1
        self._original_energy_cache.clear()  # Clear cache at start of each round
    ...
    def _evaluate_attack(self, original: str, attack: str) -> bool:
        ...
        try:
            # Check cache for original belief energy
            if original in self._original_energy_cache:
                e_orig = self._original_energy_cache[original]
            else:
                r_orig = self._engine.solve(original[:80])
                e_orig = r_orig.get("energy", 1.0) if isinstance(r_orig, dict) else 1.0
                self._original_energy_cache[original] = e_orig
