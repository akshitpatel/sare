...
_DEFAULT_DRIVES = {
    ...
    "social":        Drive("social",        level=0.25, decay_rate=0.002),
...
class HomeostaticSystem:
    ...
    def __init__(self, persist_path: Optional[Path] = None):
        ...
        # Reactive wiring: boost explore/curiosity drives on surprise_high events
        try:
            from sare.core.event_bus import get_event_bus
            def _on_surprise_high(data):
                try:
                    avg_surprise = (data or {}).get("avg_surprise", 0.0)
                    # Scale boost: +0.15 base, up to +0.30 for very high surprise
                    boost = min(0.30, 0.15 + (avg_surprise - 2.5) * 0.05)
                    self.drives["exploration"].level = min(1.0, self.drives["exploration"].level + boost)
                    self.drives["curiosity"].level   = min(1.0, self.drives["curiosity"].level   + boost * 0.5)
                    log.debug("HomeostaticSystem: surprise_high → exploration +%.2f (avg_surprise=%.2f)",
                              boost, avg_surprise)
                except Exception:
                    pass
    ...
    def on_batch_completed(self, solved_count: int, total: int):
        ...
        self.satisfy("social", 0.12)   # autonomous learning satisfies social drive
        if rate > 0.8:
